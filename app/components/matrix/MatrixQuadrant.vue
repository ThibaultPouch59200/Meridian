<template>
  <div class="flex flex-col overflow-hidden" :class="containerClass">
    <div class="mb-2 pb-[7px] border-b border-gray-200 dark:border-[#2e2e2e] flex-shrink-0">
      <div class="text-[11px] font-semibold tracking-[1px] uppercase text-black dark:text-[#f0f0ee]">{{ title }}</div>
      <div v-if="subtitle" class="text-[11px] text-gray-400 mt-[1px] italic">{{ subtitle }}</div>
    </div>
    <VueDraggable
    ...
    <button
      class="text-[12px] text-gray-300 dark:text-[#444444] cursor-pointer pt-[3px] transition-colors hover:text-gray-600 dark:hover:text-[#888888] bg-transparent border-none font-sans flex-shrink-0 text-left"
      @click="addTask"
    >
      + ajouter
    </button>
